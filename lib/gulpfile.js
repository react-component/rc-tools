...
gulp.task('test', (done) => {
  ...
  const cli_regex = /\/build\/jest\.js$/;

  if (!cli_regex.test(jestPath)) {
    console.log(
      chalk.red(
        'Jest cli not found! Please reinstall or fire a issue fot this.'
      )
    );
    done(1);
    return done(1);
  ...
  jestPath = jestPath.replace(cli_regex, '/bin/jest.js');
...
gulp.task('build', gulp.series('webpack', (done) => {
  if (fs.existsSync(resolveCwd('./examples/'))) {
...
gulp.task('js', gulp.series('cleanCompile', () => {
  console.log('compile js...');
  return babelify();
}));

gulp.task('es', gulp.series('js', () => {
...
gulp.task('publish', gulp.series('compile', (done) => {
...
gulp.task('compile_tsc', gulp.series(() => {
