...
gulp.task('browser-test-cover', (done) => {
...
gulp.task('less', ['cleanCompile'], () => {
  ...
  return gulp.src('./assets/' + '*.less')
    ...
    .pipe(through2.obj(function (file, encoding, next) {
      file.contents = new Buffer(autoprefixer.process(
        file.contents.toString(encoding)).css, encoding);
      this.push(file);
      next();
    }))
...
gulp.task('babel', ['cleanCompile'], () => {
  ...
  ret = ret.pipe(through2.obj((file, encoding, next) => {
