...
gulp.task('test', done => {
  ...
  if (argv.verbose) {
    ...
    try {
      ...
      transformerContent = transformerContent.replace('return !!config.transform && !!config.transform.length && !isIgnored;', `
      console.log('Should TRANS:', filename, config.transform, isIgnored, ignoreRegexp.toString());
