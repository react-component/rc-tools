...
gulp.task('test', done => {
  ...
  const mergedArgs = ['npx', jestPath, '--config', path.join(__dirname, './tests/ts-jest.config.js'), '--colors']
  ...
  if (argv.verbose) {
    console.log(
      chalk.yellow('Execute test:'),
      mergedArgs
    );

    try {
      const transformerPath = require.resolve('jest-runtime/build/script_transformer');
      console.log('Transformer Path >', transformerPath);
      let transformerContent = fs.readFileSync(transformerPath, 'utf8');

      transformerContent = transformerContent.replace('_getTransformPath(filename) {', `
  _getTransformPath(filename) {
    console.log('TransformPath:', filename, this._config.transform);
  `.trim());

    transformerContent = transformerContent.replace('if (willTransform) {', `
    console.log('Internal Trans:', filename, isInternalModule, isCoreModule, shouldTransform(filename, this._config), instrument);
    if (willTransform) {
    `.trim());

      fs.writeFileSync(transformerPath, transformerContent, 'utf8');
    } catch (err) {
      console.log('ERRROROROROROROROROR:', err);
    }
  }
