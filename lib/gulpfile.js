...
var libPath = new RegExp('(["\']' + pkg.name + ')\/lib\/', 'g');
...
gulp.task('build', ['webpack'], function () {
  ...
  var indexContent = '<style>li {margin:10px;}</style><h1 style="text-align: center;margin:20px;">' + pkg.name + ' examples</h1><ul>';
  for (var k in entries) {
    ...
    var html = mUtils.substitute(tpl, {
      title: path.basename(appHtml, '.html'),
      pkgName: pkg.name,
