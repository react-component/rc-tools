...
gulp.task('genTslint', done => {
  ...
  const tsContent = fs.readFileSync(tsconfig);
  fs.writeFileSync(path.join(dir, './tsconfig.json'), tsContent);
  done();
});

gulp.task('genTsJest', done => {
  const dir = resolveCwd('./');
  ...
  const jestconfig = path.join(__dirname, './ts-jest.config.js');
  const jestContent = fs.readFileSync(jestconfig);
  fs.writeFileSync(path.join(dir, './jest.config.js'), jestContent);
  done();
});

gulp.task('init-eslint', ['genPrettierrc', 'genEslint']);
gulp.task('init-tslint', ['genPrettierrc', 'genTslint', 'genTsJest']);
